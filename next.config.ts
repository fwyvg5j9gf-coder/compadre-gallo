...
const nextConfig: NextConfig = {
  ...
  async headers() {
    const csp = [
      ...
      `connect-src 'self' https://*.supabase.co https://api.stripe.com https://nominatim.openstreetmap.org https://*.clerk.com https://*.clerk.accounts.dev https://clerk-telemetry.com ${CLERK_CUSTOM} ${APP_URL}`,
      // Clerk crea un worker desde blob: para refrescar la sesión; sin esto
      // cada página llenaba la consola de errores de CSP (auditoría 26 sep 2026).
    ...
    return [
      {
        // All pages: base security headers + CSP (no X-Frame-Options here —
        // public pages like / /artistas /tienda need to be iframeable by the editor)
        source: '/(.*)',
        headers: [
      ...
      {
        // Sensitive routes: block all iframing
        source: '/(cuenta|checkout|casa)(.*)',
        headers: [
          { key: 'X-Frame-Options', value: 'DENY' },
        ],
      },
      {
        // Auth pages: Clerk needs relaxed CSP to render its UI
        source: '/cuenta/(login|registro)(.*)',
        headers: [
