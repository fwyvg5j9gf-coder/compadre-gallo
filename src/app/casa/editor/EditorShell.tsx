...
// ── Add block modal ────────────────────────────────────────────────────────────
function AddBlockModal({ pageKey, onClose }: { pageKey: string; onClose: () => void }) {
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8, maxHeight: '60vh', overflowY: 'auto' }}>
          {available.map(([type, meta]) => (
            <button key={type} disabled={pending}
              onClick={() => start(async () => { await addBlock(pageKey, type as BlockType); onClose() })}
...
// ── Block edit panel ───────────────────────────────────────────────────────────
function BlockEditPanel({ block, pageKey, onClose }: { block: Block; pageKey: string; onClose: () => void }) {
  ...
  function save() {
    start(async () => {
      await updateBlock(block.id, pageKey, values)
  ...
  return (
    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <button onClick={save} disabled={pending} className="adm-btn-primary" style={{ height: 36, fontSize: 13, marginTop: 4 }}>
...
// ── Block row ──────────────────────────────────────────────────────────────────
function BlockRow({ block, pageKey, index, total, onEdit, isEditing }:
  { block: Block; pageKey: string; index: number; total: number; onEdit: () => void; isEditing: boolean }) {
  const [pending, start] = useTransition()
  ...
  return (
    <div style={{ border: `1px solid ${isEditing ? '#003a87' : B}`, borderRadius: 8, overflow: 'hidden', background: '#fff', boxShadow: isEditing ? '0 0 0 2px rgba(0,58,135,0.12)' : 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#f6f5f1', cursor: 'pointer' }} onClick={onEdit}>
        ...
        {/* visible */}
        <button onClick={e => { e.stopPropagation(); start(() => toggleBlockVisible(block.id, pageKey, !block.visible)) }}
          disabled={pending} title={block.visible ? 'Ocultar' : 'Mostrar'}
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 15, opacity: block.visible ? 1 : 0.35, padding: '2px 4px' }}>
          {block.visible ? '👁' : '🚫'}
        </button>
        {/* move */}
        <div style={{ display: 'flex', gap: 2 }}>
          <button onClick={e => { e.stopPropagation(); start(() => moveBlock(block.id, pageKey, -1)) }}
            disabled={pending || index === 0} className="adm-btn-icon" style={{ height: 24, width: 24, fontSize: 11, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>↑</button>
          <button onClick={e => { e.stopPropagation(); start(() => moveBlock(block.id, pageKey, 1)) }}
            disabled={pending || index === total - 1} className="adm-btn-icon" style={{ height: 24, width: 24, fontSize: 11, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>↓</button>
        </div>
        {/* delete */}
        <button onClick={e => { e.stopPropagation(); if (confirm('¿Eliminar este bloque?')) start(() => deleteBlock(block.id, pageKey)) }}
          disabled={pending} className="adm-btn-icon" style={{ height: 24, width: 24, fontSize: 11, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#cc0000' }}>✕</button>
        {/* chevron */}
        <span style={{ fontSize: 11, color: M, transition: 'transform 150ms', transform: isEditing ? 'rotate(180deg)' : 'none' }}>▼</span>
      </div>
      {/* Edit panel */}
      {isEditing && <BlockEditPanel block={block} pageKey={pageKey} onClose={onEdit} />}
    </div>
  )
}
...
export default function EditorShell({ initialBlocks }: { initialBlocks: Block[] }) {
  ...
  const blocks   = initialBlocks.filter(b => b.page_key === pageKey)
  const devConf  = DEVICES.find(d => d.key === device)!

  function refreshPreview() { setIframeKey(k => k + 1) }

  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 56px)', overflow: 'hidden' }}>

      {/* ── Left: pages + device ──────────────────────────────────────────── */}
      <div style={{ width: 180, background: '#0a0a0a', display: 'flex', flexDirection: 'column', gap: 0, flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ padding: '14px 16px 8px', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>páginas</div>
        {PAGES.map(p => (
          <button key={p.key} onClick={() => { setPageKey(p.key); setEditingId(null) }}
            style={{ padding: '10px 16px', background: pageKey === p.key ? 'rgba(255,255,255,0.08)' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 13, fontWeight: 600, color: pageKey === p.key ? '#fff' : 'rgba(255,255,255,0.5)', borderLeft: `2px solid ${pageKey === p.key ? '#003a87' : 'transparent'}`, transition: 'all 120ms' }}>
            {p.label}
          </button>
        ))}
        ...
        <div style={{ padding: '8px 16px 8px', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>dispositivo</div>
        {DEVICES.map(d => (
          <button key={d.key} onClick={() => setDevice(d.key as typeof device)}
            style={{ padding: '8px 16px', background: device === d.key ? 'rgba(255,255,255,0.08)' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', fontSize: 12, color: device === d.key ? '#fff' : 'rgba(255,255,255,0.45)', transition: 'all 120ms' }}>
            {d.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, background: '#2a2a28', display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'auto', padding: '16px 0' }}>
        {/* toolbar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, background: 'rgba(255,255,255,0.06)', padding: '6px 14px', borderRadius: 8 }}>
          <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontFamily: 'monospace' }}>{page.path}</span>
          <button onClick={refreshPreview} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: 'rgba(255,255,255,0.5)', padding: '2px 6px' }} title="Actualizar preview">↻</button>
        </div>
        ...
        <div style={{
          ...
          <iframe
            ...
            ref={iframeRef}
            src={page.path}
            style={{ width: '100%', height: device === 'mobile' ? 844 : device === 'tablet' ? 1024 : 800, border: 'none', display: 'block' }}
          />
        </div>
      </div>

      {/* ── Right: blocks panel ───────────────────────────────────────────── */}
      <div style={{ width: 320, background: '#fff', borderLeft: `1px solid ${B}`, display: 'flex', flexDirection: 'column', overflow: 'hidden', flexShrink: 0 }}>
        {/* header */}
        <div style={{ padding: '14px 20px', borderBottom: `1px solid ${B}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#0a0a0a' }}>bloques · {page.label}</div>
          <button onClick={refreshPreview} title="Actualizar preview"
            style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, color: M }}>↻</button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {blocks.map((b, i) => (
            <BlockRow key={b.id} block={b} pageKey={pageKey} index={i} total={blocks.length}
        ...
        {/* add block */}
        <div style={{ padding: 16, borderTop: `1px solid ${B}` }}>
          <button onClick={() => setShowAddModal(true)} className="adm-btn-primary"
      ...
      {showAddModal && <AddBlockModal pageKey={pageKey} onClose={() => setShowAddModal(false)} />}
    </div>
  )
}
